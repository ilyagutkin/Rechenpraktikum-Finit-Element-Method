...
nu_list = [1,1e-3, 1e-4, 1e-5]  # Viskosität: Je kleiner, desto konvektionsdominanter
for nu in nu_list:      # Extrem kleine Viskosität
    ...
    g  = GlobalFunction(lambda x: 2*x[3], mesh=mesh) # div(u) = 0
    ...
    for b in range(3):
        ...
        BF.add_block_integrator(3, b, WindResWindGradQIntegrator(w, nu))
        BF.add_block_integrator(b, 3, GradPWindGradVIntegrator(w,nu))
        BF.add_block_integrator(b, 3, DivVPIntegrator())

    #BF.add_block_integrator(3,3,PressureStabilizationIntegral(delta=delta))
    BF.add_block_integrator(3,3,PressureStabilizationSUPGIntegral(w,nu,delta=delta))
    BF.assemble()
    ...
    LF.add_block_integrator(3, PressureSUPGSourceIntegral(fx, fy, fz, w, nu))
    LF.assemble()


    u = FEVectorFunction(Fes)
    u._set({0:(uex1,boundary),1:(uex2,boundary),2:(uex3,boundary)})
    res = LF.vector - BF.matrix@u.vector
    freedofs_global = Fes.get_freedofs({0: boundary, 1: boundary, 2: boundary})
    print(len(freedofs_global), " free dofs, ",len(boundary)," boundary dofs, ",Fes.ndof, " total dofs")
